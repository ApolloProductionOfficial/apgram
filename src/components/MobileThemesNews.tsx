  ...
  return (
    <div className="lg:hidden px-2 py-3 bg-card/30 backdrop-blur-md border-y border-border/30">
      <div className="grid grid-cols-2 gap-0 divide-x divide-border/50">
        {/* Левая колонка - Темы */}
        <div className="pr-1.5 space-y-1.5">
          <div className="flex items-center gap-1 pb-1 border-b border-border/30">
            <Briefcase className="h-3 w-3 text-primary" />
            <h2 className="text-[10px] font-bold text-foreground">
              {t.sidebar.themes}
            </h2>
          </div>
          <div className="space-y-1">
            {themes.map((theme, index) => (
              <button
                ...
                className="w-full text-left px-1.5 py-1.5 rounded-lg bg-card/80 hover:bg-card border border-border/50 transition-all duration-200 hover:border-primary/50 shadow-sm"
              >
                <span className="text-[9px] font-medium leading-tight line-clamp-2 text-foreground">
                  {theme.title}
                </span>
              </button>
            ))}
            ...
            {news && news.length > 0 ? (
              news.map((item) => (
                <a
                  ...
                  className="block px-1.5 py-1.5 rounded-lg bg-card/80 hover:bg-card border border-border/50 transition-all duration-200 hover:border-primary/50 shadow-sm"
                >
                  <h3 className="text-[9px] font-medium leading-tight line-clamp-2 mb-0.5 text-foreground">
                    {item.title}
                  </h3>
              ...
              <div className="px-1.5 py-2 rounded-lg bg-card/50 border border-border/30">
